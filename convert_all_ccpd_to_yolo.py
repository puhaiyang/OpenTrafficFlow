...
def extract_bbox_ccpd_standard(filename: str) -> Optional[Tuple[int, int, int, int]]:
    ...
    字段说明：
    - 字段1: Area - 面积比
    - 字段2: Tilt degree - 倾斜度 (horizontal_vertical)
    - 字段3: Bounding box - 边界框坐标 (left-up_x&left-up_y_right-bottom_x&right-bottom_y)
    - 字段4: Four vertices - 四个顶点坐标
    - 字段5: License plate number - 车牌号索引
    - 字段6-7: 其他属性
    ...
    try:
        ...
        # 解析左上角和右下角
        x0, y0 = map(int, coords[0].split('&'))  # 154&383
        x1, y1 = map(int, coords[1].split('&'))  # 386&473
